...
export default class World {
  ...
  initSharedResources() {
    this.sharedGeometries.treeTrunk = new THREE.CylinderGeometry(0.4, 0.6, 3.5, 8);
    this.sharedGeometries.treeLeaves = new THREE.ConeGeometry(2.5, 5.5, 8);

    this.sharedMaterials.treeTrunk = new THREE.MeshStandardMaterial({
      color: 0x2e1e14,
      roughness: 1.0,
    });
    this.sharedMaterials.treeLeaves = new THREE.MeshStandardMaterial({
      color: this.theme === "WARM" ? 0x0d2b0f : 0x0a1a0a,
      roughness: 1.0,
    });
    this.sharedMaterials.wallMat = new THREE.MeshStandardMaterial({
      color: 0x5d6d7e,
      roughness: 0.9,
    });
    this.sharedMaterials.buildingMat = new THREE.MeshStandardMaterial({
      color: 0x34495e,
      roughness: 0.9,
    });
    this.sharedMaterials.roofMat = new THREE.MeshStandardMaterial({
      color: 0x2c3e50,
    });
  }
  ...
  setupLights() {
    ...
    dir.castShadow = true;
    dir.shadow.mapSize.set(1024, 1024); // Dioptimalkan dari 2048x2048 ke 1024x1024
  ...
  createGround() {
    ...
    canvas.width = 1024; // Dioptimalkan dari 2048 ke 1024
    canvas.height = 1024;
    ...
    for (let i = 0; i < 1024; i += 30) {
      ctx.fillRect(i, 510, 15, 4);
      ctx.fillRect(510, i, 4, 15);
    }
  ...
  generateMap() {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      this.scene.add(tree);
      this.obstacles.push({ x, z, radius: 1.2 });
    }
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (Math.abs(x) > 10 && Math.abs(z) > 10) {
        const wallGeo = new THREE.BoxGeometry(w, 2.5, d);
        const wall = new THREE.Mesh(wallGeo, this.sharedMaterials.wallMat);
  ...
  addEnterableBuilding(cx, cz) {
    ...
    const mat = this.sharedMaterials.buildingMat;
    const h = 4.0;
  ...
  createTree() {
    ...
    const trunk = new THREE.Mesh(
      this.sharedGeometries.treeTrunk,
    ...
    const leaves = new THREE.Mesh(
      this.sharedGeometries.treeLeaves,
  ...
  dispose() {
    ...
    for (let key in this.sharedGeometries) {
      if (this.sharedGeometries[key]) this.sharedGeometries[key].dispose();
    }
    for (let key in this.sharedMaterials) {
      if (this.sharedMaterials[key]) this.sharedMaterials[key].dispose();
    }
